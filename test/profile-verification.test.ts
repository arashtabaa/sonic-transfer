...
describe('production DATA profile verification protocol', () => {
  it('round-trips a profile proposal, exact config, and probe identity', () => {
    ...
    const proposal = { protocolVersion: 1, sessionId: 123, verificationNonce: 456, profile: ModemProfileKey.TURBO, sampleRate: 48000, config, probeCount: 30 }
  ...
  it('classifies from CRC-valid ratio and preserves report metrics', () => {
    ...
    const report = { protocolVersion: 1, sessionId: 1, verificationNonce: 2, profile: ModemProfileKey.BALANCED, attemptedProbes: 30, framesDetected: 28, crcValid: 27, crcInvalid: 3, per: 0.1, classification: 'READY' as const, sampleRate: 48000 }
